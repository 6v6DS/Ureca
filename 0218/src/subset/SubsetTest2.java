...
public class SubsetTest2 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0, size = 1 << N; i < size; i++) {
			//i가 subset이므로 i를 이진수로 표현 해서
			//bit 하나가 0이면 해당 원소를 선택 X, 1이면 선택

			//bit 하나가 원소에 대한 선택 여부를 의미하므로
			//모든 bit를 다 확인할 필요 없이 원소 개수만 확인하면 된다.
			for (int j = 0; j < N; j++) {
				
				if((i & 1 << j)!=0){	//특정 위치에 1이 있는지 확인하고 싶다면
					subset[j] = 1;		//특정 위치만큼 1을 이동시켜서 확인.
				}
				// subset[j] (i>>j)&1; 도 같은 의미. if문보다 시간 절약.
			}
			//부분 집합이 완성됐으므로 부분집합 이후의 필요한 코드 작성
			print();
			
			//다음 부분 집합을 구하기 위해서
			Arrays.fill(subset, 0); // 1차원 배열에만 fill 사용가능. 2차원 배열의 경우 반복문
