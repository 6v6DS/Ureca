...
"stricNullChecks": true 설정에 의해 반드시 해당 타입의 값을 대입해야 한다.
형식] const      
      let    변수명:타입 = 값
      var
...
number      : 숫자 표현으로 정수(10진수, 16진수, 8진수), 부동소수점을 표현한다 2^52까지 표현
string      : 문자열을 의미
boolean     : 논리 값 true, false
bigint      : number 이상의 수를 표현. 숫자 뒤에 n을 붙여서 표현 ex) 123n
...
//undefined 타입이므로 값을 대입하면 오류 발생 ==> 타입 유니온에서 현재 변수에 값이 없을 때
// undefinedVal==11;
...
console.log(symbolVal === Symbol('symbol')) //결과값은 false로 나옴
