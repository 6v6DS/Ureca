...
option 
  ...
  no action  : 부모 테이블의 참조하는 데이타가 삭제되거나 수정되면 데이타는 변경되지 않는다. - 거의 사용하면 안됨
...
-- fk에 의해 부모 테이블(참조되는)에 없는 데이터를 자식 테이블(참조하는)에 
-- insert 또는 update 할 수 없다.

select * from goods;
select * from members;
insert into orders(id, gno, quantity)
values('ureca', 30, 10);	-- goods에 없는 상품번호 30번을 입력해서 오류 발생

-- fk에 의해 자식 테이블에서 참조하는 데이터를 삭제할 수 없다.
delete from goods where gno = 1;	-- orders 테이블에서 참조하고 있기 때문에 삭제시 오류 발생

select * from board;
select * from boardfile;

-- 자식(boardfile)에서 참조 하고 있어도 
-- on delete에 대해서 cascade나 set null을 설정하면 삭제할 수 있다.
delete from board where no = 1;

rollback;	-- rollback 시 데이터 복구

-- 8.0.16버전 부터 check 제약조건 됨. 이전 버전은 안됨.
create table s_emp(
...
insert into s_emp(empno, ename, salary,com_pct) values(1, '1',1000, 11);
insert into s_emp(empno, ename, salary,com_pct) values(2, '2',100, 10);
...
alter table emp3 drop foreign key fk_emp3_deptno; -- fk는 여러개일 수 있으므로 이름 작성.
...
*/
drop table members; -- 자식이 참조하고 있으면 삭제 불가
