...
public class Permutation2_nPr3_bit {
	...
	public static void permutation(int depth, int flag) {
		...
		for (int i = 0; i < N; i++) {
			//중복 검사(bitmask 이용) : i번째 원소가 사용했는지 여부를 검사하기 위해서
			//1<<i번 이동 후 flag와 &연산을 한 결과가 0이면
			//i번째 원소는 사용을 안 했고, 0이 아니면 사용함.
			if((flag & 1 << i)!=0) continue;
			numbers[depth] = input[i];
			// flag |= 1 << i 하지 않도록 주의.		// 다음 순열을 뽑을 때에도 여전히 사용중임을 표시한 상태이므로.
