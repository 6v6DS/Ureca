 ...
 *    , remove(Object o)에서 해당 기능을 수행하기 위해   equals(Object o)를 -> equals를 기준으로 찾음
...
public class ListTest {
	public static void main(String[] args) {
		...
		list.add("hello"); 		//맨 끝에 추가 -> 중복 값 상관 X.
		list.add("hello");
		list.add("java");
		list.add(3.14);
...
//		list.add(6, 256);		//IndexOutOfBoundsException 발생. 
								//-> 중간에 5번이 빠졌기 때문에 error.
		System.out.println(list);
		System.out.println(list.get(2)); // 2번째 데이터를 가져옴. get으로 추출 가능.
		...
		list2.add("hello");
		// list2.add(1);	// 타입을 지정해줬기 때문에 int는 삽입 불가. 
							// String만 저장되므로 error.

		ArrayList<Employee> list3 = new ArrayList<>();
		list3.add(new Employee("1", "1", 100000000));
		// equals 함수를 지우고 실행해보면 -1이 나옴.(찾을 수 없음)
		System.out.println(list3.indexOf(new Employee("1", "1", 100000000)));
	}
}
