...
const Average2 = () => {
  ...
  - component 내에서 state에 대한 계산 처리를 할 때 변경됐을 때만
    다시 계산하고 변경되지 않은 경우에는 기존의 값을 재사용
  - 처음에 한 번 수행되고, 
    state가 변했을 때만 callback이 호출되어 다시 계산한다.
  */
  ...
  /*
  react 에서는 함수형 Component 는 렌더링될 때마다 컴포넌트 함수를 실행시키므로
  onChange, onClick 함수는 렌더링될 때마다 새로 생성이 된다.
  ==> useCallback()을 사용한다.
  */
  const onChange = (e) => {
